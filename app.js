...
document.querySelectorAll("[data-view-tab]").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (state.activeView === "creator-tools") triggerCreatorToolsView();
  });
});
...
function channelButton(channel) {
  ...
  container.className = "channel-button-container";
  ...
  if (isAd && channel.id !== "all-in-one") {
    ...
    if (strongEl) {
      strongEl.style.paddingRight = "24px";
    }

    const deleteBtn = document.createElement("button");
    deleteBtn.className = "channel-delete-btn";
    deleteBtn.innerHTML = "×";
    deleteBtn.title = "Remove this channel";
    ...
    deleteBtn.addEventListener("click", async (e) => {
      ...
      if (confirm(`Are you sure you want to remove the channel "${channel.name}"?`)) {
        try {
          ...
          if (res.success) {
            alert(`Channel "${channel.name}" removed successfully.`);
            if (state.selectedChannelId === channel.id) {
              state.selectedChannelId = "all-in-one";
            }
            await loadDashboard({ force: true });
          }
        } catch (err) {
          alert("Failed to remove channel: " + err.message);
        } finally {
...
function showSetupOnly(status, message) {
  ...
  document.querySelector("#setupOpenaiApiKey").placeholder = status.openaiConfigured ? "Already configured" : "For OpenAI TTS models";
  document.querySelector("#setupElevenlabsApiKey").placeholder = status.elevenlabsConfigured ? "Already configured" : "For ElevenLabs TTS models";
  document.querySelector("#setupElevenlabsVoiceId").value = status.elevenlabsVoiceId || "";
...
function showDashboard() {
  ...
  const addBtn = document.querySelector("#connectChannelButton");
  if (addBtn) {
    addBtn.style.display = isAd ? "" : "none";
  }
...
async function saveLocalConfig(options = {}) {
  const config = await api("/api/config", {
    ...
    body: JSON.stringify({
      ...
      openaiApiKey: document.querySelector("#setupOpenaiApiKey")?.value || "",
      elevenlabsApiKey: document.querySelector("#setupElevenlabsApiKey")?.value || "",
      elevenlabsVoiceId: document.querySelector("#setupElevenlabsVoiceId")?.value || "",
    }),
  });
  state = { ...state, ...config };
...
/* ==========================================================================
   CREATOR TOOLS PANEL LOGIC
   ========================================================================== */

const thumbState = {
  bgType: "gradient", // solid, gradient, image
  bgValue: "linear-gradient(135deg, #1b1f24, #d91632)", // gradient preset css or solid color
  bgImage: null, // Image object for uploads
  layers: [
    {
      type: "text",
      text: "VIRAL TOPIC",
  ...
  decals: [], // elements like { type: 'arrow|circle|badge', color, val, x, y, size, rotate }
  selectedElement: null,
  dragStart: { x: 0, y: 0 },
  isDragging: false
};

let creatorToolsInitialized = false;
let voiceoverAudioUrl = null;
let currentUtterance = null;

function triggerCreatorToolsView() {
  if (!creatorToolsInitialized) {
    initCreatorTools();
    creatorToolsInitialized = true;
  }
  
  // Draw current tab
  const activeTab = document.querySelector(".tool-tab-btn.active")?.dataset.toolTab || "thumbnail";
...
function initCreatorTools() {
  ...
  document.querySelectorAll(".tool-tab-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document.querySelectorAll(".tool-tab-content").forEach(el => el.classList.add("is-hidden"));
      
      if (tab === "thumbnail") {
        document.querySelector("#toolTabThumbnail").classList.remove("is-hidden");
        drawThumbnail();
      } else if (tab === "voiceover") {
        document.querySelector("#toolTabVoiceover").classList.remove("is-hidden");
        loadAvailableVoices();
      } else if (tab === "community") {
        document.querySelector("#toolTabCommunity").classList.remove("is-hidden");
      }
    });
  });

  // 1. THUMBNAIL STUDIO: Setup Background presets
  document.querySelectorAll(".preset-bg-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      thumbState.bgType = btn.dataset.bgType;
      thumbState.bgValue = btn.dataset.bgVal;
      drawThumbnail();
    });
  });

  // Background Image Upload
  document.querySelector("#thumbBgUpload")?.addEventListener("change", (e) => {
  ...
  // Setup text layer input bindings
  const bindTextLayer = (idx) => {
    const layer = thumbState.layers[idx];
    
    document.querySelector(`#layer${idx}Active`)?.addEventListener("change", (e) => {
      layer.active = e.target.checked;
      drawThumbnail();
    });
    
    document.querySelector(`#layer${idx}Text`)?.addEventListener("input", (e) => {
      layer.text = e.target.value;
      drawThumbnail();
    });
    
    document.querySelector(`#layer${idx}Color`)?.addEventListener("change", (e) => {
      layer.color = e.target.value;
      drawThumbnail();
    });
    
    document.querySelector(`#layer${idx}Font`)?.addEventListener("change", (e) => {
      layer.font = e.target.value;
      drawThumbnail();
    });
    
    document.querySelector(`#layer${idx}Size`)?.addEventListener("input", (e) => {
      layer.size = parseInt(e.target.value) || 50;
      drawThumbnail();
    });
    
    document.querySelector(`#layer${idx}Rotate`)?.addEventListener("input", (e) => {
      layer.rotate = parseInt(e.target.value) || 0;
      drawThumbnail();
    });
    
    document.querySelector(`#layer${idx}Badge`)?.addEventListener("change", (e) => {
      layer.badge = e.target.checked;
      drawThumbnail();
    });
    
    document.querySelector(`#layer${idx}Stroke`)?.addEventListener("change", (e) => {
  ...
  // Add Decal Event Listeners
  document.querySelectorAll(".hook-add-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const type = btn.dataset.hookType;
      const color = btn.dataset.hookColor;
      const val = btn.dataset.hookVal;
      
      if (type === "arrow") {
        thumbState.decals.push({
          type: "arrow",
          color: color || "#d91632",
          x: 640,
          y: 360,
  ...
  if (canvas) {
    canvas.addEventListener("mousedown", (e) => {
      ...
      const mouseX = ((e.clientX - rect.left) / rect.width) * 1280;
      const mouseY = ((e.clientY - rect.top) / rect.height) * 720;
      
      let found = null;
      
      // Check decals first (top-most first)
      for (let i = thumbState.decals.length - 1; i >= 0; i--) {
        const decal = thumbState.decals[i];
        const dist = Math.hypot(decal.x - mouseX, decal.y - mouseY);
        if (dist < decal.size * 0.6) {
          found = decal;
          break;
        }
      }
      
      // Check text layers next
      if (!found) {
        const tempCtx = canvas.getContext("2d");
        for (let i = thumbState.layers.length - 1; i >= 0; i--) {
          const layer = thumbState.layers[i];
          if (!layer.active || !layer.text.trim()) continue;
          
          tempCtx.font = `italic 900 ${layer.size}px ${layer.font}`;
          ...
          if (dist < textWidth * 0.6 || dist < height) {
            found = layer;
            break;
          }
        }
      ...
      if (found) {
        thumbState.selectedElement = found;
        thumbState.isDragging = true;
        thumbState.dragStart = { x: mouseX - found.x, y: mouseY - found.y };
      }
    });

    canvas.addEventListener("mousemove", (e) => {
      if (thumbState.isDragging && thumbState.selectedElement) {
        const rect = canvas.getBoundingClientRect();
        const mouseX = ((e.clientX - rect.left) / rect.width) * 1280;
        const mouseY = ((e.clientY - rect.top) / rect.height) * 720;
        
        thumbState.selectedElement.x = mouseX - thumbState.dragStart.x;
        thumbState.selectedElement.y = mouseY - thumbState.dragStart.y;
        drawThumbnail();
      }
    });

    canvas.addEventListener("mouseup", () => {
      thumbState.isDragging = false;
    });

    // Double click to remove decal
  ...
  // AI Thumbnail Suggestion Brainstorm
  document.querySelector("#thumbSuggestBtn")?.addEventListener("click", async () => {
    const title = document.querySelector("#thumbVideoTitle")?.value.trim();
    if (!title) {
      alert("Please enter a video title first.");
      return;
    }
    
    const btn = document.querySelector("#thumbSuggestBtn");
    btn.disabled = true;
    btn.textContent = "Analyzing...";
    
    const container = document.querySelector("#thumbAiConceptsList");
    container.innerHTML = `<div class="empty-card-placeholder" style="color:var(--muted); font-size:12px;">Brainstorming high-CTR packaging visual layouts...</div>`;
    
    try {
      const res = await api("/api/thumbnail/suggest-concepts", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.concepts || !res.concepts.length) {
        throw new Error("No concepts returned from AI.");
      }
      
      container.innerHTML = res.concepts.map((concept, index) => `
        <div class="thumb-idea-card" data-concept-idx="${index}">
          <strong>Option ${index + 1}: "${escapeHtml(concept.textOverlay)}"</strong>
          <span><b>Layout Hook:</b> ${escapeHtml(concept.focalPoint)}</span>
          <span><b>Background:</b> ${escapeHtml(concept.backgroundStyle)}</span>
          <span><b>Decal/Circle:</b> ${escapeHtml(concept.visualHook)}</span>
          <span style="margin-top: 4px; font-style: italic; font-size: 10px; color: var(--blue);">${escapeHtml(concept.reasoning)}</span>
        </div>
      `).join("");
      
      // Click concept to apply to controls
      container.querySelectorAll(".thumb-idea-card").forEach((card) => {
        card.addEventListener("click", () => {
          const idx = parseInt(card.dataset.conceptIdx);
          const concept = res.concepts[idx];
          
          // Apply overlay text
          const words = concept.textOverlay.split(/\s+/);
          const line1 = words.slice(0, Math.ceil(words.length / 2)).join(" ");
          const line2 = words.slice(Math.ceil(words.length / 2)).join(" ");
          
          document.querySelector("#layer0Text").value = line1 || "VIRAL TOPIC";
          thumbState.layers[0].text = line1 || "VIRAL TOPIC";
          
          document.querySelector("#layer1Text").value = line2 || "";
          thumbState.layers[1].text = line2 || "";
          thumbState.layers[1].active = Boolean(line2);
          document.querySelector("#layer1Active").checked = Boolean(line2);
          
          // Suggest arrow or focus ring based on visualHook
          thumbState.decals = [];
          if (concept.visualHook.toLowerCase().includes("arrow")) {
            thumbState.decals.push({
              type: "arrow",
              color: "#d91632",
              x: 850,
          ...
          } else if (concept.visualHook.toLowerCase().includes("circle") || concept.visualHook.toLowerCase().includes("ring")) {
            thumbState.decals.push({
              type: "circle",
              color: "#d91632",
              x: 850,
          ...
          // Reset alignment for text so it sits nicely on left
          thumbState.layers[0].x = 420;
          thumbState.layers[1].x = 420;
          
          drawThumbnail();
          alert(`Applied text overlay: "${concept.textOverlay}" and set background layout recommendations.`);
        });
      });
      
    } catch (err) {
      alert("Failed to get suggestions: " + err.message);
      container.innerHTML = `<div class="empty-card-placeholder" style="color:var(--red); font-size:12px;">Error: ${escapeHtml(err.message)}</div>`;
    } finally {
      btn.disabled = false;
      btn.textContent = "Brainstorm Ideas";
    }
  });

  // Reset Canvas
  document.querySelector("#resetThumbnailBtn")?.addEventListener("click", () => {
    thumbState.bgType = "gradient";
    thumbState.bgValue = "linear-gradient(135deg, #1b1f24, #d91632)";
    thumbState.bgImage = null;
    thumbState.decals = [];
    ...
    document.querySelector("#layer0Text").value = "VIRAL TOPIC";
    document.querySelector("#layer0Active").checked = true;
    document.querySelector("#layer0Color").value = "#ffffff";
    document.querySelector("#layer0Font").value = "Impact";
    document.querySelector("#layer0Size").value = 70;
    document.querySelector("#layer0Rotate").value = -5;
    document.querySelector("#layer0Badge").checked = true;
    document.querySelector("#layer0Stroke").checked = true;

    document.querySelector("#layer1Text").value = "DON'T MISS THIS";
    document.querySelector("#layer1Active").checked = true;
    document.querySelector("#layer1Color").value = "#ffff00";
    document.querySelector("#layer1Font").value = "Impact";
    document.querySelector("#layer1Size").value = 50;
    document.querySelector("#layer1Rotate").value = 2;
    document.querySelector("#layer1Badge").checked = true;
    document.querySelector("#layer1Stroke").checked = true;
    
    drawThumbnail();
  });

  // Download Canvas as PNG
  document.querySelector("#downloadThumbnailBtn")?.addEventListener("click", () => {
    const canvas = document.getElementById("thumbnailCanvas");
    if (!canvas) return;
    ...
    a.href = url;
    a.download = "thumbnail_1280x720.png";
    a.click();
  });

  // 2. VOICEOVER GENERATOR: Character & word count tracking
  const textarea = document.querySelector("#voiceText");
  textarea?.addEventListener("input", () => {
    const text = textarea.value.trim();
    document.querySelector("#voiceCharCount").textContent = `${text.length} characters`;
    document.querySelector("#voiceWordCount").textContent = `${text ? text.split(/\s+/).length : 0} words`;
  });

  // Slider visual updates
  document.querySelector("#voiceRate")?.addEventListener("input", (e) => {
    document.querySelector("#voiceRateVal").textContent = `${e.target.value}x`;
  });
  
  document.querySelector("#voicePitch")?.addEventListener("input", (e) => {
    document.querySelector("#voicePitchVal").textContent = e.target.value;
  });

  // Speak / Preview
  document.querySelector("#voiceSpeakBtn")?.addEventListener("click", () => {
    speakLocalVoiceover();
  });

  // Stop Speak
  document.querySelector("#voiceStopBtn")?.addEventListener("click", () => {
    window.speechSynthesis.cancel();
    document.querySelector("#speakingWave").classList.remove("active");
    document.querySelector("#speakingTextIndicator").textContent = "Playback stopped.";
  });

  // Download Audio proxy
  document.querySelector("#voiceDownloadBtn")?.addEventListener("click", () => {
    exportVoiceoverAudio();
  });

  // 3. COMMUNITY POSTS: Generation
  document.querySelector("#generateCommunityPostsBtn")?.addEventListener("click", async () => {
    const topic = document.querySelector("#communityPostTopic")?.value.trim();
    const postType = document.querySelector("#communityPostType")?.value;
    const channelContext = document.querySelector("#communityChannelNiche")?.value.trim();
    
    if (!topic) {
      alert("Please enter a topic for community posts.");
      return;
    }
    
    const btn = document.querySelector("#generateCommunityPostsBtn");
    btn.disabled = true;
    btn.textContent = "Writing posts...";
    
    const container = document.querySelector("#communityPostsList");
    container.innerHTML = `<div class="empty-card-placeholder" style="color:var(--muted); text-align:center;">AI is drafting 3 viral community posts (polling formats, quizzes)...</div>`;
    
    try {
      const res = await api("/api/community-posts/generate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ topic, postType, channelContext })
      });
      ...
      container.innerHTML = res.posts.map((post, idx) => {
        let pollHtml = "";
        
        if (post.pollOptions && post.pollOptions.length > 0) {
          pollHtml = `
            <div class="yt-post-poll">
              ${post.pollOptions.map((opt, oIdx) => {
                const isCorrect = post.quizAnswerIndex === oIdx ? "correct-option" : "";
                return `
                  <div class="yt-poll-option ${isCorrect}">
                    <div class="yt-poll-bg" style="width: ${oIdx === 0 ? "42%" : oIdx === 1 ? "28%" : "15%"}"></div>
                    <span class="yt-poll-label">${escapeHtml(opt)}</span>
                    <span class="yt-poll-percent">${oIdx === 0 ? "42%" : oIdx === 1 ? "28%" : "15%"}</span>
                  </div>
                `;
              }).join("")}
            </div>
        ...
        const explanationHtml = post.explanation ? `
          <div class="yt-quiz-explanation">
            💡 <b>Insight:</b> ${escapeHtml(post.explanation)}
          </div>
        ` : "";
        
        const firstLetter = (channelContext || "Y").charAt(0).toUpperCase();
        
        return `
          <div class="youtube-post-card" data-post-idx="${idx}">
            <div class="copy-post-badge">Click card to copy post</div>
            <div class="yt-card-header">
              <div class="yt-avatar">${firstLetter}</div>
              <div class="yt-channel-meta">
                <span class="yt-channel-name">${escapeHtml(channelContext || "Your YouTube Channel")}</span>
                <span class="yt-post-time">Scheduled (AI Recommended)</span>
              </div>
            </div>
            <div class="yt-post-content" id="postTextCopy_${idx}">${escapeHtml(post.postText)}</div>
            ${pollHtml}
            ${explanationHtml}
            <div class="yt-card-actions">
              <div class="yt-action-item">👍 2.4k likes</div>
              <div class="yt-action-item">💬 42 comments</div>
            </div>
          </div>
        `;
      }).join("");
      
      // Bind copy listeners to cards
      container.querySelectorAll(".youtube-post-card").forEach((card) => {
        card.addEventListener("click", async () => {
          const idx = parseInt(card.dataset.postIdx);
          const post = res.posts[idx];
          
          let copyString = post.postText;
          if (post.pollOptions && post.pollOptions.length > 0) {
            copyString += "\n\n[Poll Options:]\n" + post.pollOptions.map((opt, oIdx) => {
              const marker = post.quizAnswerIndex === oIdx ? " (Correct)" : "";
              return `${oIdx + 1}. ${opt}${marker}`;
            }).join("\n");
          }
          if (post.explanation) {
            copyString += `\n\n[Explanation/Context:]\n${post.explanation}`;
          ...
          try {
            ...
            const badge = card.querySelector(".copy-post-badge");
            if (badge) badge.textContent = "Copied to clipboard!";
            setTimeout(() => {
              card.classList.remove("copied");
              if (badge) badge.textContent = "Click card to copy post";
            }, 2000);
          } catch (err) {
            alert("Failed to copy post text.");
          }
        });
      });
      
    } catch (err) {
      container.innerHTML = `<div class="empty-card-placeholder" style="color:var(--red);">Error: ${escapeHtml(err.message)}</div>`;
    } finally {
      btn.disabled = false;
      btn.textContent = "Generate Engaged Posts";
    }
  });
}

function speakLocalVoiceover() {
  window.speechSynthesis.cancel();
  
  const text = document.querySelector("#voiceText").value.trim();
  if (!text) {
    alert("Please enter a script script first.");
    return;
  }
  
  const voiceName = document.querySelector("#voiceSelect").value;
  const rate = parseFloat(document.querySelector("#voiceRate").value);
  const pitch = parseFloat(document.querySelector("#voicePitch").value);
  ...
  // Track words spoken and highlight
  const spokenDisplay = document.querySelector("#spokenWordDisplay");
  const words = text.split(/\s+/);
  spokenDisplay.innerHTML = words.map(w => `<span>${escapeHtml(w)}</span>`).join(" ");
  ...
  currentUtterance.onboundary = (event) => {
    if (event.name === "word") {
      ...
      const spanNodes = spokenDisplay.querySelectorAll("span");
      spanNodes.forEach((node, idx) => {
        node.classList.toggle("highlight", idx === wordIdx);
      });
    }
  };
  
  currentUtterance.onstart = () => {
    document.querySelector("#speakingWave").classList.add("active");
    document.querySelector("#speakingTextIndicator").textContent = "Synthesizing vocal track (playing)...";
  };
  
  currentUtterance.onend = () => {
    document.querySelector("#speakingWave").classList.remove("active");
    document.querySelector("#speakingTextIndicator").textContent = "Playback finished.";
  };
  
  currentUtterance.onerror = (err) => {
    document.querySelector("#speakingWave").classList.remove("active");
    document.querySelector("#speakingTextIndicator").textContent = "Vocal track error.";
    console.error(err);
  };
...
async function exportVoiceoverAudio() {
  ...
  const langMatch = selectedOption ? selectedOption.textContent.match(/\[(.*?)\]/) : null;
  const lang = langMatch ? langMatch[1].split("-")[0] : "en";
  
  const indicator = document.getElementById("speakingTextIndicator");
  indicator.textContent = "Synthesizing full voiceover (natural chunked output)...";
  
  const btn = document.getElementById("voiceDownloadBtn");
  btn.disabled = true;
  
  try {
    const res = await fetch("/api/voiceover/export", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    indicator.textContent = "Audio exported successfully! Check downloads.";
  } catch (error) {
    indicator.textContent = "Export error: " + error.message;
